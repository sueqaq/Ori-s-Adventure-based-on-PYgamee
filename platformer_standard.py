...
# load sounds
pygame.mixer.music.load('img/Enamel.wav')
...
def reset_level(level):
    if level == 1:
        player.reset(100, screen_height - 700)
    elif level == 2:
        ...
        player.reset(70, screen_height - 50)
    blob_group.empty()
...
#######################jump_image#######################
            ...
            screen.blit(game_over_img, ((screen_width // 2) -
                        200, (screen_height // 2)-200))
            if self.rect.y > 200:
                self.rect.y -= 5
    ...
    def reset(self, x, y):
        ...
        self.dead_image = pygame.image.load('img/Dead3.png')
        self.dead_image = pygame.transform.scale(self.dead_image, (50, 60))
        self.jump_image_right = pygame.image.load('img/jump1.png')
        self.jump_image_right = pygame.transform.scale(
            self.jump_image_right, (60, 75))
        self.jump_image_left = pygame.image.load('img/jump2.png')
        self.jump_image_left = pygame.transform.scale(
            self.jump_image_left, (60, 75))
        self.jump_image_left = pygame.transform.flip(
            self.jump_image_left, True, False)
        self.image = self.images_right[self.index]
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = y
...
#######################
if level == 1:
    player = Player(50, screen_height - 700)
elif level == 2:
    ...
    player = Player(150, screen_height - 100)
elif level == 6:
    player = Player(70, screen_height - 50)
...
restart_button = Button(screen_width // 2 - 100,
                        screen_height // 2 + 150, restart_img)
...
while run:
    ...
    if main_menu == True:
        ...
        if game_over == 0:
            ...
            #print("Number of coins remaining:", len(coin_group))
